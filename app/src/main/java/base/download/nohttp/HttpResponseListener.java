...
public class HttpResponseListener<T> implements OnResponseListener<T> {
    ...
    public void onFailed(int what, Response<T> response) {
        ...
        if (exception instanceof NetworkError) {// 网络不好
            ToastShowUtil.showTextToast(mActivity, R.string.error_please_check_network);
        } else if (exception instanceof TimeoutError) {// 请求超时
            ToastShowUtil.showTextToast(mActivity, R.string.error_timeout);
        } else if (exception instanceof UnKnownHostError) {// 找不到服务器
            ToastShowUtil.showTextToast(mActivity, R.string.error_not_found_server);
        } else if (exception instanceof URLError) {// URL是错的
            ToastShowUtil.showTextToast(mActivity, R.string.error_url_error);
        } else if (exception instanceof NotFoundCacheError) {
            // 这个异常只会在仅仅查找缓存时没有找到缓存时返回
            // 没有缓存一般不提示用户，如果需要随你。
        } else {
            ToastShowUtil.showTextToast(mActivity, R.string.error_unknow);
        }
